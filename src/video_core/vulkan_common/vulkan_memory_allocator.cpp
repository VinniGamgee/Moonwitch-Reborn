...
MemoryCommit::MemoryCommit(VmaAllocator alloc, VmaAllocation a,
    ...
    if (GPU::Logging::IsActive() && Settings::values.gpu_log_memory_tracking.GetValue()) {
        GPU::Logging::GPULogger::GetInstance().LogMemoryAllocation(
            reinterpret_cast<uintptr_t>(memory), static_cast<u64>(size), 0);
    }
}
...
MemoryCommit::MemoryCommit(MemoryCommit&& rhs) noexcept
    : allocator{std::exchange(rhs.allocator, nullptr)},
      ...
      offset{std::exchange(rhs.offset, 0)}, size{std::exchange(rhs.size, 0)},
...
std::span<u8> MemoryCommit::Map() {
    ...
    const size_t n = static_cast<size_t>(
        std::min<VkDeviceSize>(size, (std::numeric_limits<size_t>::max)()));
...
vk::Buffer MemoryAllocator::CreateBuffer(const VkBufferCreateInfo& ci, MemoryUsage usage) const {
    pressure_manager.Refresh();
    const auto anv_flags = (usage == MemoryUsage::Stream &&
